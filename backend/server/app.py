...
async def lifespan(app: FastAPI):
    ...
    logger.info("agent Researcher API ready - local mode (no database persistence)")
    yield
    # Shutdown
...
async def agent_discovery(request: Request):
    """Advertise agent Researcher services via the Agent Discovery Protocol."""
    ...
    if research_request.report_type != "multi_agents":
        ...
        response = {
            ...
            "research_information": {
                ...
                "research_step_costs": researcher.get_step_costs(),
                "cost_estimation": researcher.get_cost_estimation_details(),
