...
This guide specifically focuses on how to integrate your locally running agent Researcher MCP server with the Claude desktop application for Mac, providing a seamless research experience within the Claude interface.
...
1. agent Researcher MCP server installed and running locally
...
### 1. Install and Run the agent Researcher MCP Server

Make sure you have the agent Researcher MCP server installed and running:
...
2. **Edit the Configuration File**:
   ...
   - Add or update the `mcpServers` section to include your local agent Researcher MCP server:
...
- Clicking this icon should display the agent Researcher tools provided by your MCP server
- If you don't see the hammer icon, check the Claude application logs for any errors

## Using agent Researcher in Claude Desktop
...
- Learn about [additional configuration options](../gptr/config) for the agent Researcher
