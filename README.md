...
**agent Researcher 是一个智能体代理，专为各种任务的综合在线研究而设计。**

代理可以生成详细、正式且客观的研究报告，并提供自定义选项，专注于相关资源、结构框架和经验报告。受最近发表的[Plan-and-Solve](https://arxiv.org/abs/2305.04091) 和[RAG](https://arxiv.org/abs/2005.11401) 论文的启发，agent Researcher 解决了速度、确定性和可靠性等问题，通过并行化的代理运行，而不是同步操作，提供了更稳定的性能和更高的速度。

**我们的使命是利用人工智能的力量，为个人和组织提供准确、客观和事实的信息。**

## 为什么选择agent Researcher?
...
**研究任务平均耗时约 3 分钟**，具体成本取决于所选用的 LLM 提供商和模型配置。
...
agent Researcher 支持 **LangSmith** 以增强链路追踪和可观测性，特别适用于调试和优化复杂的多智能体工作流。
...
本项目 "agent Researcher "是一个实验性应用程序，按 "现状 "提供，不做任何明示或暗示的保证。我们根据 MIT 许可分享用于学术目的的代码。本文不提供任何学术建议，也不建议在学术或研究论文中使用。
...
**请注意，使用大型语言模型可能会因消耗 API tokens 而产生费用**。使用本项目即表示您承认有责任监控和管理自己的 API 使用情况及相关费用。强烈建议您定期检查所使用的 LLM 提供商的 API 使用情况，并设置任何必要的限制或警报，以防止发生意外费用。
